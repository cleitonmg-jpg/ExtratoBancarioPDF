...
3. Coolify configura o banco internamente — você não precisa adicionar nenhuma variável neste serviço manualmente
4. **Copie a `DATABASE_URL`** exibida na tela do banco criado — você vai precisar dela no passo 4

> Formato da URL gerada pelo Coolify:
> `postgres://USUARIO:SENHA@nome-do-servico:5432/NOME_DO_BANCO`

> **As variáveis de ambiente ficam APENAS no serviço da aplicação (Extrato), não no serviço do PostgreSQL.**
...
> Todas as variáveis abaixo são inseridas **no serviço da aplicação (Extrato)**, na aba **Environment Variables**.
> No serviço do PostgreSQL não é necessário adicionar nada.

Na aba **Environment Variables** do serviço **Extrato**, adicione:
